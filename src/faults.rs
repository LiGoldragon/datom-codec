...
impl Conceivable<Datom> for Extent {
    ...
    fn conceive(&self) -> Result<protos::Situated<Datom>, Self::Fault> {
        Ok(Situated(Situation { extent: Extent(0, 0), children: vec![] }, Datom::Struct(vec![self.0.conceive()?.1, self.1.conceive()?.1])))
    }
}
...
impl Conceivable<Datom> for Locus {
    ...
    fn conceive(&self) -> Result<protos::Situated<Datom>, Self::Fault> {
        Ok(Situated(Situation { extent: Extent(0, 0), children: vec![] }, Datom::Struct(vec![self.path.conceive()?.1, self.extent.conceive()?.1])))
    }
}
...
impl Conceivable<Datom> for protos::Fault {
    ...
    fn conceive(&self) -> Result<protos::Situated<Datom>, Self::Fault> {
        Ok(Situated(Situation { extent: Extent(0, 0), children: vec![] }, Datom::Struct(vec![self.extent.conceive()?.1, self.problem.conceive()?.1])))
    }
}
...
impl Conceivable<Datom> for Problem {
    ...
    fn conceive(&self) -> Result<protos::Situated<Datom>, Self::Fault> {
        let datom = match self {
            Self::Shape(expected, found) => "Shape".carrying(Datom::Struct(vec![expected.conceive()?.1, found.conceive()?.1])),
            Self::Arity(expected, found) => "Arity".carrying(Datom::Struct(vec![expected.conceive()?.1, found.conceive()?.1])),
            ...
            Self::Exhausted => Datom::Word(Word::try_from("Exhausted").expect("Exhausted is a word")),
            Self::BudgetExhausted => Datom::Word(Word::try_from("BudgetExhausted").expect("BudgetExhausted is a word")),
        };
        Ok(Situated(Situation { extent: Extent(0, 0), children: vec![] }, datom))
    }
}
...
impl Conceivable<Datom> for Fault {
    ...
    fn conceive(&self) -> Result<protos::Situated<Datom>, Self::Fault> {
        let datom = match self {
            ...
            Self::Conceptual(locus, problem) => "Conceptual".carrying(Datom::Struct(vec![locus.conceive()?.1, problem.conceive()?.1])),
            Self::Corporate(locus, problem) => "Corporate".carrying(Datom::Struct(vec![locus.conceive()?.1, problem.conceive()?.1])),
        };
        Ok(Situated(Situation { extent: Extent(0, 0), children: vec![] }, datom))
    }
}
