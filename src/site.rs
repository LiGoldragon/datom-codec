...
use crate::anatomy::{Budgeted, Datom, Expected, Fault, Found, IncorporationBudget, Locus, Problem};
...
impl Refusing for Variant<'_> {
    fn refuse(&self, problem: Problem) -> Fault {
        Fault::Corporate(Locus { path: vec![], extent: self.at.extent }, problem)
    }
}
...
impl<'a> Sited<'a> for Site<'a> {
    ...
    fn variant(self) -> Result<Variant<'a>, Fault> {
        ...
        match datom {
            ...
            _ => Err(Fault::Corporate(Locus { path: vec![], extent: at.extent }, Problem::Shape(Expected::Variant, datom.found()))),
        }
    }
...
impl<T: Datomic> Incorporable<T> for Datom {
    ...
    fn incorporate(&self, at: &Situation, mut budget: Self::Budget) -> Result<T, Fault> {
        Site { datom: self, at, budget: &mut budget }.corporate()
    }
}
